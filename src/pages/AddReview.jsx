    ...
    try {
      ...
      if (result?.error) {
        if (result.error.includes('already reviewed')) {
          toast.error('You already reviewed this shop');
      ...
      toast.success(`Review submitted! You earned ${pointsEarned} points${isVerified ? ' (Verified ✓)' : ''}`);
      navigate(`/shops/${shopId}`);
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-card rounded-2xl border border-border/50 p-6 space-y-4">
          ...
          {!isNewShop ? (
            ...
            <div className="space-y-3">
              ...
              <button type="button" onClick={() => setIsNewShop(false)} className="text-sm text-muted-foreground hover:text-foreground">← Choose existing shop</button>
            </div>
          )}
        </div>
        ...
        <div className="bg-card rounded-2xl border border-border/50 p-6 space-y-4">
          ...
          <Textarea placeholder="Tell others about your experience with this shop..." value={comment} onChange={(e) => setComment(e.target.value)} rows={5} className="rounded-xl resize-none" />
        </div>
        ...
        <div className="bg-card rounded-2xl border border-border/50 p-6 space-y-4">
          ...
          <p className="text-xs text-muted-foreground">Add a link to a screenshot or post as proof — earns extra points and a Verified badge ✓</p>
          <Input placeholder="https://..." value={evidenceUrl} onChange={(e) => setEvidenceUrl(e.target.value)} className="rounded-xl" />
          {evidenceUrl.trim() && <p className="text-xs text-emerald-600 font-medium">✓ Verified review — you'll earn {pts.verified} points instead of {pts.review}</p>}
        </div>

        <Button type="submit" disabled={submitting} className="w-full h-12 rounded-2xl text-base font-semibold gap-2">
          {submitting ? <><Loader2 className="h-5 w-5 animate-spin" /> Submitting…</> : <><Send className="h-5 w-5" /> Submit Review</>}
        </Button>
      </form>
    </div>
  );

  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-card rounded-2xl p-6 w-full max-w-sm space-y-4 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="bg-secondary/50 rounded-xl p-3 text-sm text-muted-foreground italic line-clamp-2">
          "{text.slice(0, 80)}"
        </div>
        <div className="flex items-center gap-2 text-xs text-primary">
          <BadgeCheck className="h-4 w-4" /> Shared by a Verified TrustPulse Reviewer
        </div>
          ...
          {comments.map((c, i) => (
            <div key={c.id || i} className="flex gap-2">
              ...
              <div className="flex-1 bg-secondary/40 rounded-xl px-3 py-2">
                ...
                {c.created_at && <p className="text-[10px] text-muted-foreground/50 mt-1">{new Date(c.created_at).toLocaleDateString('en-AE', { month: 'short', day: 'numeric' })}</p>}
              </div>
            </div>
          ))}
          {currentUser ? (
            <div className="flex gap-2 mt-2">
              ...
              <div className="flex-1 flex gap-1.5">
                <input
                  ...
                  className="flex-1 bg-secondary/40 rounded-xl px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-primary/20 border border-border/30 focus:border-primary/40 transition-all"
                />
                <button
    ...
    <>
      ...
      <div className="bg-card rounded-2xl border border-border/50 p-5 space-y-4 hover:shadow-md transition-all hover:border-border">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {(review.reviewer_trust >= 75 || review.reviewer_verified) && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold text-blue-600 bg-blue-50 dark:bg-blue-950/30 px-2 py-0.5 rounded-full border border-blue-200 dark:border-blue-800">
        ...
        <div className={cn(
          ...
          reviewText ? 'bg-secondary/30 text-foreground/80 border border-border/30' : 'text-muted-foreground italic'
        )}>
        ...
        {/* Evidence link */}
        {review.evidence_url && (
          <a href={review.evidence_url} target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 text-xs text-primary hover:underline bg-primary/5 px-3 py-1.5 rounded-lg border border-primary/10"
          >
            📎 View evidence
          </a>
        )}
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <button
            ...
            disabled={liking}
            className={cn(
              'flex items-center gap-1.5 text-xs font-medium transition-all px-2.5 py-1.5 rounded-lg',
          ...
          {canShare && (
            <button onClick={() => setShowShare(true)}
              className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors px-2 py-1.5 rounded-lg hover:bg-secondary/50"
            >
          ...
          {currentUser && (
            <button onClick={handleReport}
              className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-destructive transition-colors ml-auto px-2 py-1.5 rounded-lg hover:bg-destructive/5"
            >
